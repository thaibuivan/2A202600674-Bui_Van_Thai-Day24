...
def chunk_semantic(text: str, threshold: float = SEMANTIC_THRESHOLD,
    ...
    try:
        ...
        print("  ⚠️  sentence_transformers not installed. Returning basic chunks.")
...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    for para in paragraphs:
        if len(current_parent_text) + len(para) > parent_size and current_parent_text:
            ...
            parents.append(Chunk(
                ...
                metadata={**metadata, "chunk_type": "parent", "strategy": "hierarchical", "parent_id": pid}
            ))
            current_parent_text = ""
    ...
    for parent in parents:
        ...
        sentences = [s.strip() for s in parent_text.replace("\n\n", " ").split(". ") if s.strip()]
...
def chunk_structure_aware(text: str, metadata: dict | None = None) -> list[Chunk]:
    ...
    for part in parts:
        ...
        if re.match(r'^#{1,3}\s+', part):
            if current_content:
                full_text = f"{current_header}\n\n{current_content}" if current_header else current_content
                chunks.append(Chunk(
